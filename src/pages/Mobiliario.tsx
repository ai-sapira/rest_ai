...
export default function Mobiliario() {
  ...
  // Filter anuncios for mobiliario category
  const mobiliarioAnuncios = anuncios.filter(anuncio => 
    anuncio.categoria.toLowerCase() === 'mobiliario'
  );
  ...
  // Apply filters
  const filteredAnuncios = mobiliarioAnuncios.filter(anuncio => {
    const matchesPrice = priceFilter === "todos" || 
      (priceFilter === "bajo" && anuncio.precio && parseInt(anuncio.precio) < 1000) ||
      (priceFilter === "medio" && anuncio.precio && parseInt(anuncio.precio) >= 1000 && parseInt(anuncio.precio) <= 3000) ||
      (priceFilter === "alto" && anuncio.precio && parseInt(anuncio.precio) > 3000);
  ...
  return (
    <main className="flex-1 p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        {/* Header */}
        <div className="space-y-2">
          <h1 className="text-3xl font-bold">Mobiliario</h1>
          <p className="text-muted-foreground">
            Encuentra mesas, sillas, barras y todo el mobiliario para tu restaurante
          </p>
        ...
        {filteredAnuncios.length > 0 ? (
          ...
          <Card>
            <CardContent className="p-8 text-center">
              ...
              <p className="text-muted-foreground">
                No hay anuncios de mobiliario que coincidan con tus filtros
              </p>
              <Button 
                variant="outline" 
                className="mt-4"
